...
public class TurnTimer implements EventHandler {
    ...
    public void turnStarted(TurnStarted turnStarted) {
        ...
        reminder = scheduledExecutorService
            .schedule(() -> eventBus.dispatch(turnWillEndAt),
                    TURN_TIMEOUT_IN_SECONDS - 20, TimeUnit.SECONDS);
        forceEndTurn = scheduledExecutorService
            .schedule(
                    () -> eventBus.dispatch(new PlayerRanOutOfTime(turnStarted.getNewPlayer())),
