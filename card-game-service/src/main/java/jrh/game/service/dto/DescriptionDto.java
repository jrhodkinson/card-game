...
public class DescriptionDto {
    ...
    public static class Factory {
        ...
        private List<DescriptionLine> descriptionLines(Description description) {
            return description
                .getAtomicDescriptions()
                .stream()
                .map(this::descriptionTokens)
        ...
        private List<DescriptionToken> descriptionTokens(AtomicDescription atomicDescription) {
            return atomicDescription
                .getPieces()
                .stream()
                .map(dp -> new DescriptionToken(dp.get(descriptionContext),
                        dp.getType(), dp.getHelp(descriptionContext).orElse(null)))
                .collect(toList());
        }
    }
